...
{
    [TestClass]
    public class DISAssetSellingControllerTests
    {
        ...
        public void A_DISAssetSelling_AddDetailEditReportMemo_GetPPNValueByMemoCreatedDate_IsValidData()
        {
            //Arrange       
            string[] getConfig = ConfigurationManager
                .AppSettings["DISAssetSelling_AddDetailEditReportMemo_GetPPNValueByMemoCreatedDate_IsValidData"]
                .Split(',');

            var createdDateMemo = DateTime.Parse(getConfig[0].ToString());

            LoadAllPPNValueSupportData(createdDateMemo);

            //Expect
            var PPNValue = int.Parse(getConfig[1].ToString());

            //Act
            var result = Decimal.ToInt16(_calculationService.OPLTAxSettingGetValue(createdDateMemo, TaxType.OptNameTaxType, TaxType.TaxTypePPN));
        ...
        [TestMethod]
        [TestCategory("S0266337")]
        public void B_DISAssetSelling_AddDetailEditReportMemo_GetPPHValueByMemoCreatedDate_IsValidData()
        {
            //Arrange       
            string[] getConfig = ConfigurationManager
                .AppSettings["DISAssetSelling_AddDetailEditReportMemo_GetPPHValueByMemoCreatedDate_IsValidData"]
                .Split(',');

            var createdDateMemo = DateTime.Parse(getConfig[0].ToString());

            LoadAllPPHValueSupportData(createdDateMemo);

            //Expect
            var PPHValue = int.Parse(getConfig[1].ToString());

            //Act
            var result = Decimal.ToInt16(_calculationService.OPLTAxSettingGetValue(createdDateMemo, TaxType.OptNameTaxType, TaxType.TaxTypePPH23));
