...
function countAmountToBePaid(counterRow) {
    ...
    while (ppnVal == 0) {
        if ($('#ppnNowValue').val() !== undefined) {
            ppnVal = parseInt($('#ppnNowValue').val());

            var offer = document.getElementById('OfferPriceAmount_' + counterRow).value;
            var ppn = (parseFloat(offer.replace(/,/g, ""))) * parseFloat(ppnVal / 100);
...
function countAuctionFee(counterRow) {
    ...
    while (ppnVal == 0 || pphVal == 0) {
        if ($('#ppnNowValue').val() !== undefined && $('#pphNowValue').val() != undefined) {
            ppnVal = parseInt($('#ppnNowValue').val());
            pphVal = parseInt($('#pphNowValue').val());
            ...
            var ppn = ((parseFloat(mobilisationFee.replace(/,/g, ""))) + (parseFloat(auctionFee.replace(/,/g, "")))) * parseFloat(ppnVal / 100);
            var pph = ((parseFloat(mobilisationFee.replace(/,/g, ""))) + (parseFloat(auctionFee.replace(/,/g, "")))) * parseFloat(pphVal / 100);
...
function countOfferPrice(counterRow) {
    ...
    while (ppnVal == 0)
    {
        if ($('#ppnNowValue').val() !== undefined)
        {
            ...
            var offerPrice = parseFloat(paid.replace(/,/g, "")) / parseFloat((ppnVal + 100) / 100);
            var offerPriceFormat = (Math.round(offerPrice)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
...
//Add by Agung 23/09/2020
function popupDispEmpMemo() {
    $('#updateDispEmpMemo').modal('show');

}
//end by Agung 23/09/2020

//Add by Agung 14/01/2021
function popupDispEmpProfit() {
    $('#updateDispEmpProfit').modal('show');

}
//end by Agung 14/01/2021
...
//Add by Agung 23/09/2020
function UpdateDispEmpMemo() {
    var modalData = {};
    var HeadOfBranch = $("#HeadOfBranch").val();
    var RegionalManager = $("#RegionalManager").val();
    var MemoCategory = $("#MemoReportType").val();
    var IdTb_DIS_AssetSelling = $("#IdTb_DIS_AssetSellingData").val();
    var IdDispApprovalEmp = $("#IdDispApprovalEmp").val();
    var CreatedBy = $("#CreatedBy").val();
    var CreatedDate = $("#CreatedDate").val();

    modalData["MemoCategory"] = MemoCategory;
    modalData["HeadOfBranch"] = HeadOfBranch;
    modalData["RegionalManager"] = RegionalManager;
    modalData["IdTb_DIS_AssetSelling"] = IdTb_DIS_AssetSelling;
    modalData["IdDispApprovalEmp"] = IdDispApprovalEmp;
    ...
    $.ajax(
        {
            ...
            contentType: "application/json",
            data: JSON.stringify(modalData),
            url: serverRoot + "DISAssetSelling/UpdateDispEmpMemo",
            success: function(response) {
                $('#updateDispEmpMemo').modal('hide');
            }
        });
}
//End by Agung 23/09/2020
    ...
    $.ajax(
        {
            ...
            contentType: "application/json",
            data: JSON.stringify(modalData),
            url: serverRoot + "DISAssetSelling/UpdateDispEmpProfit",
            success: function(response) {
                $('#updateDispEmpProfit').modal('hide');
            }
        });
}
//End by Agung 14/01/2021
